...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["otp_input"]) && isset($_POST["newpass"])) {
    ...
    if (!isset($_SESSION["otp"]) || !isset($_SESSION["otp_time"])) {
        ...
        if (isValidPassword($_POST["newpass"])){
            ...
            $user = $_SESSION['seller_uname'];
            $update_sql = "UPDATE seller SET PASSWORD = ? WHERE USERNAME = ?";
            $update_stmt = mysqli_prepare($conn, $update_sql);
            mysqli_stmt_bind_param($update_stmt, "ss", $hashedPassword, $user);
            if (mysqli_stmt_execute($update_stmt)) {
                unset($_SESSION["seller_uname"], $_SESSION['seller_pass'], $_SESSION['seller_email']);
                echo "<script>alert('Password reset successfully!'); window.location.replace('../login.php');</script>";
                exit;
            } else {
                unset($_SESSION["seller_uname"], $_SESSION['seller_pass'], $_SESSION['seller_email']);
                echo "<script>alert('Password reset failed!'); window.location.replace('../login.php');</script>";
                exit;
            }
        } else {
    ...
    try {
        ...
        $mail->addAddress($_SESSION['seller_email'] ?? '');
        ...
        $mail->Body = '
                    ...
                    <body>
                        <div class="container">
                            ...
                            <div class="content">
                                <p>Dear User,</p>
                                <p>We have received a request to change your password for seller account. To complete the account creation process, please use the following code:</p>
                                <p class="otp">' . $otp . '</p>
  ...
  <div class="modal-dialog">
    <form method="POST" class="modal-content">
      <div class="modal-body">
        ...
        <input type="text" name="otp_input" class="form-control" required style="border: 2px solid  #007bff" maxlength="6">
        <br>
